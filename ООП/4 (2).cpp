...
public:
	...
	void fillClass(int sizeOfBase, student* student, std::ofstream& out) {
	...
	int printToFile(int sizeOfBase, student* student, std::ofstream& out) {
		if (out.is_open()) {
			for (size_t i = 0; i < sizeOfBase; ++i) {
...
int main() {
	...
	std::ofstream outf(path, ios::out | ios::binary);
	...
	stu->fillClass(sizeOfBase, stu, outf);
	stu->printToConsole(sizeOfBase, stu);

	int n;
	std::cout << "Enter the year of study: ";
	std::cin >> n;

	stu->printToFile(sizeOfBase, stu, outf);
	stu->countOfExcStudents(sizeOfBase, n, stu, outf);
